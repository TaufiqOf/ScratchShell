...
public class Buffer
{
    private CircularList<BufferLine> lines;
    ...
    private int getCorrectBufferLength(int rows)
    {
        if (!hasScrollback)
            return rows;
    ...
    private bool IsReflowEnabled => hasScrollback;// && Terminal.Options.WindowsMode;
    ...
    private void Reflow(int newCols, int newRows)
    {
        if (cols == newCols)
        {
            return;
