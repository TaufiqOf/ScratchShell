...
namespace XtermSharp
{
    ...
    {
        private readonly ReadingBuffer readingBuffer;
        ...
        //
        private void SelectCharset(string p)
        {
            if (p.Length != 2)
        ...
        // ESC # NUMBER
        //
        private void DoubleWidthSingleHeight()
        {
        }
        ...
        // dhtop
        //
        private void SetDoubleHeightTop()
        { }

        // dhbot
        private void SetDoubleHeightBottom()
        { }          // dhbot

        //
        // swsh
        //
        private void SingleWidthSingleHeight()
        { }
        ...
        //
        private void SelectDefaultCharset()
        {
        ...
        //
        private void KeypadNumericMode()
        {
        ...
        //
        private void KeypadApplicationMode()
        {
        ...
        private void SetTitleAndIcon(string data)
        {
        ...
        /// </summary>
        private void SetIconTitle(string data)
        {
            terminal.SetIconTitle(data ?? string.Empty);
        }
        ...
        //
        private void EraseInBufferLine(int y, int start, int end, bool clearWrap = false)
        {
        ...
        // @param y row index
        //
        private void ResetBufferLine(int y)
        {
        ...
        //
        private void SetCursorStyle(int[] pars, string collect)
        {
            if (collect != " ")
                return;
        ...
        //     Ps.
        //
        private void CharAttributes(int[] pars)
        {
        ...
        private void ResetMode(int[] pars, string collect)
        {
            if (pars.Length == 0)
                return;
        ...
        private void SetMode(int[] pars, string collect)
        {
            if (pars.Length == 0)
                return;
        ...
        //
        private void TabClear(int[] pars)
        {
        ...
        //   [1,1]) (HVP).
        //
        private void HVPosition(int[] pars)
        {
            int p = 1;
            int q = 1;
            if (pars.Length > 0)
        ...
        // reuse CSI Ps B ?
        //
        private void VPositionRelative(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void LinePosAbsolute(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void RepeatPrecedingCharacter(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
        ...
        //reuse CSI Ps C ?
        //
        private void HPositionRelative(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void CharPosAbsolute(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void EraseChars(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
        ...
        //
        private void ScrollDown(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void ScrollUp(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void DeleteLines(int[] pars)
        {
            var p = Math.Max(pars.Length == 0 ? 1 : pars[0], 1);
            var buffer = terminal.Buffer;
            var row = buffer.Y + buffer.YBase;
        ...
        //
        private void EraseInLine(int[] pars)
        {
            var p = pars.Length == 0 ? 0 : pars[0];
            var buffer = terminal.Buffer;
            switch (p)
        ...
        //     Ps = 2  -> Selective Erase All.
        //
        private void EraseInDisplay(int[] pars)
        {
            var p = pars.Length == 0 ? 0 : pars[0];
            var buffer = terminal.Buffer;
            int j;
        ...
        //
        private void CursorForwardTab(int[] pars)
        {
        ...
        // reuse CSI Ps A ?
        //
        private void CursorPrecedingLine(int[] pars)
        {
            int param = Math.Max(pars.Length > 0 ? pars[0] : 1, 1);
            var buffer = terminal.Buffer;
        ...
        // same as CSI Ps B?
        //
        private void CursorNextLine(int[] pars)
        {
            int param = Math.Max(pars.Length > 0 ? pars[0] : 1, 1);
            var buffer = terminal.Buffer;
        ...
        //
        private void InsertChars(int[] pars)
        {
        ...
        private void PrintStateReset()
        {
            readingBuffer.Reset();
        }

        private unsafe void Print(byte* data, int start, int end)
        {
